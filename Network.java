...
public class Network {
    ...
    public User getUser(String name) {
        ...
        for (int i=0 ; i<userCount; i++){
            if (users[i].getName().toLowerCase().equalsIgnoreCase(name.toLowerCase())){
                return users[i];
        }
    }
        return null;
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i=0 ; i<userCount; i++){
            ...
            if (users[i].countMutual(getUser(name))>maxMutuals) {
                recommended=users[i].getName();
                maxMutuals=users[i].countMutual(getUser(name));
            }
        } 
    ...
    public String mostPopularUser() {
        ...
        for (int i=0 ; i<userCount; i++){
            if (followeeCount(users[i].getName())>followeeCount(mostPopular)){
